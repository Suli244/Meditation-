import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';

const font = 'Syne';

abstract class AppTextStylesMeditation {
  static TextStyle s10Wbold({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 10,
        fontWeight: FontWeight.bold,
      );
  static TextStyle s12W400({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 12.sp,
        fontWeight: FontWeight.w400,
      );
  static TextStyle s11W400({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 11,
        fontWeight: FontWeight.w400,
      );
  static TextStyle s12Wbold({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 12,
        fontWeight: FontWeight.bold,
      );
  static TextStyle s14W400({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 14,
        fontWeight: FontWeight.w400,
      );
  static TextStyle s14W500({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 14,
        fontWeight: FontWeight.w500,
      );
  static TextStyle s14Wbold({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 14,
        fontWeight: FontWeight.bold,
      );
  static TextStyle s15W400({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 15.sp,
        fontWeight: FontWeight.w400,
      );
  static TextStyle s15W600({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 15,
        fontWeight: FontWeight.w600,
      );
  static TextStyle s16W400({Color? color, double? fontSize}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: fontSize ?? 16,
        fontWeight: FontWeight.w400,
      );
  static TextStyle s16W500({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 16,
        fontWeight: FontWeight.w500,
      );
  static TextStyle s16WBold({Color? color}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: 16,
        fontWeight: FontWeight.bold,
      );
  static TextStyle s18W700({Color? color}) => TextStyle(
        fontSize: 18,
        fontWeight: FontWeight.w700,
        color: color ?? Colors.black,
        fontFamily: font,
      );
  static TextStyle s18W500({Color? color}) => TextStyle(
        fontSize: 18,
        fontWeight: FontWeight.w500,
        color: color ?? Colors.black,
        fontFamily: font,
      );
  static TextStyle s19W600({Color? color}) => TextStyle(
        fontSize: 19.sp,
        fontWeight: FontWeight.w600,
        color: color ?? Colors.black,
        fontFamily: font,
      );
  static TextStyle s25W500({Color? color}) => TextStyle(
        fontSize: 25,
        fontWeight: FontWeight.w500,
        color: color ?? Colors.black,
        fontFamily: font,
      );
  static TextStyle s18Wbold({Color? color}) => TextStyle(
        fontSize: 18,
        fontWeight: FontWeight.bold,
        color: color ?? Colors.black,
        fontFamily: font,
      );
  static TextStyle s17W400({Color? color, double? fontSize}) => TextStyle(
        color: color ?? Colors.black,
        fontFamily: font,
        fontSize: fontSize ?? 17,
        fontWeight: FontWeight.w400,
      );
  static TextStyle s17W600({Color? color}) => TextStyle(
        fontSize: 17,
        fontWeight: FontWeight.w600,
        color: color ?? Colors.black,
        fontFamily: font,
      );

  static TextStyle s24W900({Color? color}) => TextStyle(
        fontSize: 24,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w900,
      );
  static TextStyle s24W700({Color? color}) => TextStyle(
        fontSize: 24,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w700,
      );
  static TextStyle s24W600({Color? color}) => TextStyle(
        fontSize: 24,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w600,
      );
  static TextStyle s26W600({Color? color}) => TextStyle(
        fontSize: 26.sp,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w600,
      );
  static TextStyle s20W700({Color? color}) => TextStyle(
        fontSize: 20,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w700,
      );
  static TextStyle s34W600({Color? color}) => TextStyle(
        fontSize: 34,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w600,
        height: 1.20,
      );
  static TextStyle s46W700({Color? color}) => TextStyle(
        fontSize: 46,
        color: color ?? Colors.black,
        fontFamily: font,
        fontWeight: FontWeight.w700,
      );
}
